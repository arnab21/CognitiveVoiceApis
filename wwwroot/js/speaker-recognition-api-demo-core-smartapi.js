        ...
        request.onload = function () {
            ...
            if (json.status == 'succeeded') {
                ...
                if (speaker != null && speaker.length > 0) {
                    ...
                    $("#speakerName").val(speaker[0].name);
                    $("#speakerId").val(speaker[0].profileId);
                } else {
...
function addAudioPlayer(blob) {
    ...
    var log = document.getElementById('speakerIdentificationLog');
...
(function () {
    ...
    var old = console.log;
    var logger = document.getElementById('speakerIdentificationLog');
...
if (typeof (Storage) !== "undefined") {
    var storedProfileIds = JSON.parse(localStorage.getItem('profileIds'));
    if (storedProfileIds.length > 0) {
        for (i = 0; i < storedProfileIds.length; i++) {
            var speaker = storedProfileIds[i];
            console.log('Initializing profile : ' + speaker.name);
            profileIds.push(speaker);
        }
    }
    else {
        localStorage.setItem('profileIds', JSON.stringify(profileIds));
